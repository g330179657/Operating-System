...
public class KThread {
	...
	public static void finish() {
		...
		KThread parent = currentThread.getParent();
		if(parent != null){
			parent.ready();
		}
		sleep();
	...
	public void join() {
		...
		KThread parent = this.getParent();
		Lib.assertTrue(parent == null);

		if(this.getStatus() == statusFinished){
			Machine.interrupt().restore(intStatus);
			return;
		}

		this.setParent(currentThread());
		currentThread().sleep();
	}
	public KThread getParent(){
		return this.parent;
	}
	public boolean setParent(KThread parent){
		this.parent = parent;
		return true;
	}
	...
	protected boolean isBlocked(){
		return this.status == statusBlocked;
	}
	...
	private static void joinTest1(){
		KThread child1 = new KThread(new Runnable(){
			public void run(){
				for(int i = 0; i < 10; i++){
					System.out.println("I (heart) Nachos!" + "thread name: " + currentThread.getName());
		...
		for(int i = 0; i < 5; i++){
			System.out.println("busy..." + "thread name: " + currentThread.getName());
			KThread.currentThread().yield();
		}
	...
	private static void joinTest2(){
		...
		KThread parent1 = new KThread(new Runnable(){
			public void run(){
				System.out.println("parent1 call join with child1");
				child1.join();
		...
		KThread parent2 = new KThread(new Runnable(){
			public void run(){
				System.out.println("parent2 call join with child2");
				child2.join();
				System.out.println("child2 done.");
			}
		});
		parent1.fork(); child1.fork();
		parent2.fork(); child2.fork();
		parent1.join();
		parent2.join();
